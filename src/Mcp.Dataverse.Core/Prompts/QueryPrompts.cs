...
public class QueryPrompts
{
    ...
    public static ChatMessage RecentRecords([Description("The table's logical name e.g. contact, account")] string tableName) =>
        new(ChatRole.User, $"Retrieve 50 recent records for {tableName}, sorted by modifiedon descending");
    ...
    new(ChatRole.User, $"""
        Retrieve all the solutioncomponent records for solution with uniquename for {solutionUniqueName}. 
